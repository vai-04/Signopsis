...
Runs the whole sign -> text layer in one process, no browser or server:
    ...
    python -m setu.perceive.camera_cv --user me [--camera 0] [--lang en] [--left-handed] [--speak]
...
def main():
    ...
    status = "ready"
    caption = ""
    print("SETU camera client running. q = quit, r = reset, t = teach, 1/2/3 = answer repair")
    while True:
        ok, bgr = cap.read()
        if not ok:
            break
        ...
        if not a.no_window:
            ...
            cv2.imshow("SETU sign -> text", view)
