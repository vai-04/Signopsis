"""Fit L2 trust weights and report the section-12 metrics for sign -> text.
    ...
    python -m setu.fuse.fit                 # fit + write fuse/weights.json + eval/sign_report.json
    python -m setu.fuse.fit --eval-only     # evaluate the current weights
    python -m setu.fuse.fit --n 400 --plot  # bigger run, reliability diagram PNG

Data: simulated camera streams (setu.perceive.simcam) of random ISL
...
from setu.fuse.trust import DEFAULT, EMIT_AT, WEIGHTS_PATH, SlotSignals, TrustModel, sigmoid
