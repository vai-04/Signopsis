// SETU 3D signer page logic: text -> sign playback, mirror-me (webcam retargeting), pose lab, styling.
...
const DEMO = window.SETU_AVATAR_DEMO || null;       // offline build embeds precomputed plans
...
$("snap").onclick = () => {
  ...
  a.download = "setu-avatar.png"; a.href = canvas.toDataURL("image/png"); a.click();
};
...
async function fetchPlan(text, resolutions) {
  if (DEMO) {
    ...
    throw new Error("The offline demo only has the example sentences. Run the SETU server for free text.");
  }
...
$("simMirror").onclick = async () => {
  ...
  if (!r.ok) { $("mirrorInfo").textContent = "Simulator needs the SETU server."; return; }
...
status(DEMO ? "offline demo" : "connected to SETU");
requestAnimationFrame(tick);
window.__setu = { avatar, player, camera, controls, setView, loadPlan, signText, switchMode, labVals, syncLab };
signText();
