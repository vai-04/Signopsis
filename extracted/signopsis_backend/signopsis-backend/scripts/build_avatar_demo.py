"""Build a single-file, offline copy of the 3D signer (no server needed).
    ...
    python scripts/build_avatar_demo.py          # -> dist/signopsis_avatar3d_demo.html
...
STATIC = ROOT / "signopsis" / "serve" / "static"
...
def main(out: Path = ROOT / "dist" / "signopsis_avatar3d_demo.html") -> Path:
    results = {}
    ...
    html = html.replace('<script type="module" src="/static/avatar3d/app.js"></script>',
                        f"<script>window.SIGNOPSIS_AVATAR_DEMO={data};</script>\n<script type=\"module\">{bundle}</script>")
    html = html.replace('<nav><a href="/">Hub</a><a href="/text-to-sign">Text → sign (2D)</a><a href="/sign-to-text">Sign → text</a></nav>',
                        '<nav><span style="color:var(--muted);font-size:13px">offline demo · run the SIGNOPSIS server for free text and the webcam</span></nav>')
