# SIGNOPSIS architecture and execution (v0.2)
...
sess = SignSession(user="me")                 # MemoryStore under ./data (SIGNOPSIS_DATA to move it)
for msg in wire_frames: events = sess.handle(msg)
```

The OpenCV client (`signopsis/perceive/camera_cv.py`), the tests and the WebSocket all use exactly this call.
...
| Simulated trust data | Recorded, labelled clips | `python -m signopsis.fuse.fit` (same code) |
...
uvicorn signopsis.serve.main:app             # http://localhost:8000 → Sign → text → Start camera
pytest -q                               # 160 tests, about 45 s (the 3D avatar tests need Node)
```

Without a webcam: open the page, choose **Simulated**, or run
`python -m signopsis.perceive.simulate "I went to the bank yesterday." --severity 0.5`.

Native (no browser): `python -m signopsis.perceive.camera_cv --user me`, after
...
`python -m signopsis.fuse.fit --n 400 --plot` → `signopsis/eval/sign_report.json` and `sign_reliability.png`.
...
## Web app (signopsis-web)

The production frontend lives in its own repo (React 19, Vite, @react-three/fiber, Framer Motion, Tailwind v4).
All screens call one facade (`src/services/backend.js`) that switches between this server and an in-browser
replica built from fixtures exported by `signopsis-web/scripts/export_fixtures.py`. Contracts are mirrored in
`src/contracts/contracts.d.ts`; `src/services/adapters.js` maps `gate` → the four UI trust words
(Clear = emit, Enhanced = emit helped by context/memory/an answer, Checking = repair, Held = hold) so the main UI
never shows raw scores. Layer → screen map: L0 CameraStage/ConsentSheet · L1 live HUD + partial gloss ·
L2 TrustBar/TrustBadge · L3 CaptionStack/GlossStrip · L4 AvatarStage, Compose, Live stage · L5 RepairCard,
Teach a sign, Library. Pipeline E (Screen Glance) uses `POST /api/screen/describe` with a structural DOM
snapshot, never pixels.
