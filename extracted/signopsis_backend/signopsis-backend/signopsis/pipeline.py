...
from signopsis.generate.avatar2d import build_timeline, frames_json
from signopsis.generate.signs import FS_DUR, LEXICON, fs_letters, has_sign
from signopsis.resolve import vocab as V
from signopsis.resolve.rules import nonmanual_for, text_to_frame
from signopsis.schemas import (CaptionTarget, GlossItem, NMKey, RenderPlan, Repair,
                          RepairOption, SemanticFrame, SignTarget)

EMIT_AT = 0.75
HOLD_BELOW = 0.40
EMIT_AT_HIGH_STAKES = 0.85
ADVISORY = ("Medical / legal context detected. SIGNOPSIS is a supplement, not a replacement: "
...
def _resolve(text: str, resolutions: dict[int, str]) -> SemanticFrame:
    """L3 dispatch. SIGNOPSIS_RESOLVER=ollama tries the local LLM, falls back to rules."""
    if os.getenv("SIGNOPSIS_RESOLVER", "rules") == "ollama":
        from signopsis.resolve.llm import llm_frame
