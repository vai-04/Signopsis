...
from signopsis.generate.hand2d import HAND_SCALE
from signopsis.generate.signs import ALPHABET, FS_DUR, LEXICON
from signopsis.schemas import GlossItem, LatticeSlot, SignTarget

K = 12                 # resample length
WRIST_W = 3.0          # weight of absolute hand location vs handshape
TAU = 0.08             # softmax temperature (refit with: python -m signopsis.eval.calibrate)
...
class Recognizer:
    ...
    recognizer the live camera uses (signopsis.perceive.recognizer), restricted to
    ...
    def classify(self, R, L, kind: str = "sign") -> list[tuple[str, float]]:
        if self.legacy:
            ...
            from signopsis.perceive.recognizer import Index, features as f2, resample as r2, smooth_seq
            idx = _shared_index()
...
@lru_cache(maxsize=1)
def _shared_index():
    from signopsis.perceive.recognizer import Index
    return Index()
