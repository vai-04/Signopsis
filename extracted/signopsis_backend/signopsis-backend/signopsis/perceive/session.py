...
the same JSON messages.
  ...
  {"type":"enroll_undo"}   (drops the last recorded sample)
...
from signopsis.fuse.trust import (COVERAGE_HOLD, COVERAGE_REPAIR, SlotSignals, TrustModel, gate_for, hold_reason,
                             thresholds)
from signopsis.memory.store import MemoryStore
from signopsis.perceive.landmarks import Normalizer, Obs
from signopsis.perceive.nonmanual import Baseline, analyse
from signopsis.perceive.recognizer import K, Index, dtw_many, dtw_open, resample, resample_batch
from signopsis.perceive.segment import (PhraseTracker, Prepared, Seg, decode_phrase, is_active, noise_floor,
                                   prepare, trim_edges, trim_travel)
from signopsis.resolve import vocab as V
from signopsis.resolve.sign_to_text import SlotIn, decode_context, high_stakes, realize
from signopsis.schemas import (CaptionTarget, Entity, Grounding, LatticeSlot, NonManual, PerceptEvent, Prosody,
...
ADVISORY = ("Medical / legal words detected. SIGNOPSIS is a supplement, not a replacement: "
...
class SignSession:
    ...
    def handle(self, msg: dict) -> list[dict]:
        try:
            ...
            if kind == "enroll_start":
                return self._enroll_start(msg.get("label", ""), [], _meta(msg))
            if kind == "enroll_from_unknown":
                pre = [(q, d) for q, d, _, _ in self.unknowns[-2:]]
                self.unknowns.clear()
                return self._enroll_start(msg.get("label", ""), pre, _meta(msg))
            if kind == "enroll_undo":
                if not self.enroll:
                    return [{"type": "error", "message": "not enrolling"}]
                if self.enroll["samples"]:
                    self.enroll["samples"].pop()
                return [{"type": "enroll_progress", "label": self.enroll["label"], "display": self.enroll["display"],
                         "have": len(self.enroll["samples"]), "need": ENROLL_SHOTS, "undone": True}]
    ...
    def _enroll_start(self, label: str, pre: list, meta: Optional[dict] = None) -> list[dict]:
        norm = normalise_label(label)
        self.enroll = {"label": norm, "display": label.strip(), "samples": list(pre), "meta": meta or {}}
    ...
    def _enroll_sample(self, frames: list[Obs]) -> list[dict]:
        ...
        n = self.store.add_sign(self.user, e["label"], [q for q, _ in e["samples"]], [d for _, d in e["samples"]],
                                display=e["display"], extra=e.get("meta"))
        self._reload_index()
...
SIGN_KINDS = ("name", "variant", "jargon")
SIGN_SCOPES = ("me", "team")


def _meta(msg: dict) -> dict:
    """Optional enrollment metadata sent by the SIGNOPSIS web app (kind of sign, who may use it)."""
    out = {}
    if msg.get("kind") in SIGN_KINDS:
        out["kind"] = msg["kind"]
    if msg.get("scope") in SIGN_SCOPES:
        out["scope"] = msg["scope"]
    return out
