...
from signopsis.schemas import (Entity, Grounding, NonManual, PerceptEvent, LatticeSlot,
                          Prosody, SemanticFrame, Unresolved, new_id, now_ms)
...
def text_to_frame(text: str, resolutions: Optional[dict[int, str]] = None) -> tuple[SemanticFrame, PerceptEvent]:
    """Resolve raw text into a SemanticFrame.

    `resolutions` maps *token index* -> chosen gloss (from a repair tap).
    Returns the frame plus the synthetic text PerceptEvent it is grounded on.
    """
    ...
    percept = PerceptEvent(
        t0=t0, t1=t0, channel="text", source="keyboard", lang_hint=lang,
        lattice=[LatticeSlot(slot=i, cands=[(t.text, 1.0)]) for i, t in enumerate(toks)],
    )
    ...
    for t in content:
        ti = toks.index(t)
        if t.gloss:
            items.append([t.gloss, V.CATEGORY.get(t.gloss, "NOUN"), t, ti])
        elif t.cands:
    ...
    for gi, (g, cat, t, ti) in enumerate(items):
        if t is None:
            continue
        grounding.append(Grounding(span=t.span, percept_ids=[percept.id], t=(t0, t0), gloss_index=gi))
        if g.startswith("FS:"):
    ...
    if text.endswith("!"):
        intensity = 0.8
        affect = "urgent" if any(g in ("HELP", "PAIN", "HOSPITAL", "NOW") for g in gloss) else "happy"
    ...
    frame = SemanticFrame(
        ...
        prosody=Prosody(affect=affect, intensity=intensity, emphasis=emphasis,
                        pace="fast" if affect == "urgent" else "normal"),
        grounding=grounding, unresolved=unresolved, gloss=gloss,
        high_stakes=high, trust=round(trust, 3),
    )
