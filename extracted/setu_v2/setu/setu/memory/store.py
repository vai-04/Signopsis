...
Set SETU_QDRANT_URL (or SETU_QDRANT=":memory:") to also mirror prototypes
...
from setu.perceive.recognizer import Proto

DATA_ROOT = Path(os.getenv("SETU_DATA", Path(__file__).resolve().parents[2] / "data"))
    ...
    def add_sign(self, user: str, label: str, samples: list[np.ndarray], durs: list[float],
        ...
        with self._lock:
            ...
            for T, d in new:
                ...
                meta.append({"created": now, "display": display or label})
...
def _maybe_qdrant():
    url = os.getenv("SETU_QDRANT_URL") or os.getenv("SETU_QDRANT")
    if not url:
        return None
    try:
