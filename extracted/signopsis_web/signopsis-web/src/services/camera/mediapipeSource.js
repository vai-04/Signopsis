// SIGNOPSIS camera client: webcam -> MediaPipe (in the browser) -> WireFrame messages for /ws/sign.
// Raw video never leaves the page. Only landmarks, a few face blendshapes and one brightness number are sent.
//
// MediaPipe is loaded at runtime (not bundled): first from the backend's local vendor copy
// (`python scripts/fetch_models.py` on the backend), then from the public CDN / model bucket.
import { config } from "../../config.js";

const TV_VERSION = "1.0.1";
const B = () => config.apiBase;
const LOCAL_TV = () => `${B()}/static/vendor/tasks-vision`;
const CDN_TV = `https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@${TV_VERSION}`;
const MODELS = () => ({
  hand: [`${B()}/static/vendor/models/hand_landmarker.task`,
         "https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task"],
  pose: [`${B()}/static/vendor/models/pose_landmarker_lite.task`,
         "https://storage.googleapis.com/mediapipe-models/pose_landmarker/pose_landmarker_lite/float16/1/pose_landmarker_lite.task"],
  face: [`${B()}/static/vendor/models/face_landmarker.task`,
...
export async function loadVision(log = () => {}) {
  let mod, base;
  const local = LOCAL_TV();
  if (await exists(`${local}/vision_bundle.mjs`)) {
    mod = await import(/* @vite-ignore */ `${local}/vision_bundle.mjs`);
    base = (await exists(`${local}/wasm/vision_wasm_internal.wasm`)) ? `${local}/wasm` : `${CDN_TV}/wasm`;
    log(`MediaPipe JS: backend copy · wasm: ${base === `${CDN_TV}/wasm` ? "CDN" : "backend"}`);
  } else {
    mod = await import(/* @vite-ignore */ `${CDN_TV}/vision_bundle.mjs`);
    base = `${CDN_TV}/wasm`;
...
export class CameraSource {
  ...
  async init() {
    ...
    this.log(`models: ${[hand, pose, face].every(u => !u.startsWith("https://storage")) ? "local" : "remote"}`);
...
export function drawLandmarks(ctx, frame, { clear = true, background = null } = {}) {
  ...
  if (frame.pose) {
    ctx.strokeStyle = "rgba(242,242,242,.75)";
    ...
    if (n && (n[3] ?? 1) > .4) { ctx.fillStyle = "rgba(242,242,242,.9)"; ctx.beginPath(); ctx.arc(X(n), Y(n), W / 90, 0, 7); ctx.fill(); }
  }
  frame.hands.forEach((h, i) => {
    const right = /left/i.test(h.label) !== !!frame.mirrored;    // person's right hand
    ctx.strokeStyle = right ? "#D96868" : "#91AE6E";
