...
Enable with:  SETU_RESOLVER=ollama  SETU_OLLAMA_MODEL=qwen3:4b
...
from setu.schemas import SemanticFrame
...
def _ollama(prompt: str, timeout: float = 4.0) -> str:
    url = os.getenv("SETU_OLLAMA_URL", "http://localhost:11434/api/generate")
    body = json.dumps({"model": os.getenv("SETU_OLLAMA_MODEL", "qwen3:4b"), "prompt": prompt,
...
def llm_frame(text: str, resolutions: dict[int, str],
    ...
    frame.gloss = cand
    frame.provenance.resolver = os.getenv("SETU_OLLAMA_MODEL", "qwen3:4b")
    return frame
