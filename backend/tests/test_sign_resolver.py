"""L3 (sign direction): text -> ISL gloss. Word order and non-manual rules from grammar/isl.md."""

import pytest

from setu.perceive.fake import fake_percept
from setu.resolve.isl_rules import nonmanual_for, text_to_frame
...
@pytest.mark.parametrize("text,expected", [
    ...
    ("Good morning, I need help.", ["GOOD", "MORNING", "ME", "NEED", "HELP"]),  # rule 8: greeting stays whole
...
def test_question_types_and_negation():
    ...
    assert nonmanual_for(f).head == "shake"
    assert nonmanual_for(text_to_frame("What is your name?")[0]).brow == "furrowed"
...
def test_ambiguity_is_not_guessed():
    ...
    assert u.reason == "lexical_ambiguity"
    assert f.trust < 0.75
...
def test_grounding_covers_every_gloss():
    """Every output gloss points back at a source span."""
    text = "I went to the bank yesterday."
    f, pe = text_to_frame(text)
    assert pe.channel == "text"
...
def test_speech_text_is_grounded_on_the_speech_percept():
    pe = fake_percept(0, t0=1000)
    text = " ".join(pe.top1())
    f, used = text_to_frame(text, source=pe, speaker_id="spk_0")
    assert used is pe
    assert all(g.percept_ids == [pe.id] and g.t == (pe.t0, pe.t1) for g in f.grounding)
    assert f.speaker_id == "spk_0" and f.provenance["source"] == "fake"
...
def test_empty_and_garbage_still_validate():
    for t in ("", "?!"):
        f, _ = text_to_frame(t)
        assert f.gloss == [] and f.grounding       # SemanticFrame requires grounding
