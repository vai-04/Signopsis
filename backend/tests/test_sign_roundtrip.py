"""The round-trip gate: read the rendered avatar back before trusting it."""
...
def plan(*gl):
    return SignTarget(gloss=[GlossToken(g=g, dur_ms=LEXICON[g]["dur"], fs_fallback="-".join(g)) for g in gl])
...
def test_gate_passes_clear_signs():
    ...
    assert all(s.cands and s.cands[0].score >= s.cands[-1].score for s in res.lattices)
