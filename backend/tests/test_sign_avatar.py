...
from setu.generate.avatar2d import build_timeline, frames_json, render_offline
from setu.generate.fingerspell import ALPHABET, fs_duration, fs_letters
from setu.generate.hand2d import HANDSHAPES, HandPose, pose_to_landmarks
from setu.generate.signs import LEXICON
from setu.resolve.isl_vocab import CATEGORY
from setu.schemas import GlossToken, SignTarget
...
def test_every_sign_stays_in_frame():
    for g, s in LEXICON.items():
        t, R, L, gidx, _ = render_offline(SignTarget(gloss=[GlossToken(g=g, dur_ms=s["dur"])]))
        for H in (R, L):
...
def test_timeline_and_frames():
    tgt = SignTarget(gloss=[GlossToken(g="ME", dur_ms=380),
                            GlossToken(g="FS:AB", dur_ms=0, fingerspelled=True, fs_fallback="A-B"),
                            GlossToken(g="GO", dur_ms=460)])
    ...
    assert {"Rq", "Lq", "face"} <= set(fj["frames"][0])                   # 3D rig + non-manuals


def test_alphabet_complete():
    assert set("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") <= set(ALPHABET)
    assert fs_letters("Priya-2!") == "P-R-I-Y-A-2"
    assert fs_duration("A-B") == 2 * 280 + 60
