...
(landmarks in, lattice out). Swap `Recognizer` for the ST-GCN prototype
lookup in Phase 4; the gate code does not change.
"""
...
from .. import config
from ..schemas import Candidate, GlossToken, LatticeSlot, SignTarget
from .avatar2d import render_offline
from .fingerspell import ALPHABET, FS_DUR
from .hand2d import HAND_SCALE
from .signs import LEXICON

K = 12                 # resample length
WRIST_W = 3.0          # weight of absolute hand location vs handshape
TAU = 0.08             # softmax temperature (refit once eval/ has read-back labels)
...
THRESHOLD = config.ROUNDTRIP_THRESHOLD
THRESHOLD_HIGH_STAKES = config.ROUNDTRIP_THRESHOLD_HIGH_STAKES
...
def _isolated(g: str, fingerspell: bool) -> np.ndarray:
    if fingerspell:
        item = GlossToken(g="FS", dur_ms=FS_DUR, fingerspelled=True, fs_fallback=g)
    else:
        item = GlossToken(g=g, dur_ms=LEXICON[g]["dur"])
...
class Recognizer:
    """Landmarks in, lattice out. Same contract as the real L1 recognizer."""
    ...
    def classify(self, R, L, kind: str = "sign") -> list[tuple[str, float]]:
        ...
        return [(bank.labels[i], float(p[i])) for i in order]
...
def warmup() -> None:
    """Build both template banks (the first call is the slow one)."""
    template_bank("sign")
    template_bank("letter")
...
def roundtrip_gate(target: SignTarget, threshold: float = THRESHOLD, seed: int = 7,
    ...
    for gi, item in enumerate(target.gloss):
        ...
        lattice_slots.append(LatticeSlot(slot=gi, cands=[Candidate(value=g, score=round(p, 3)) for g, p in cands[0]]))
        tops = rb.get(gi, ["?"])
