...
Drop-in stand-in for the VRM avatar (Phase 7). `render_offline` is what the
round-trip gate calls; `frames_json` is what the dev viewer plays. Frames
also carry the parametric hand pose (`Rq`/`Lq`) so a 3D rig can be IK-driven
...
from ..schemas import NMKey, SignTarget
from .fingerspell import ALPHABET, FS_DUR, FS_GLIDE
from .hand2d import REST_L, REST_R, HandPose, pose_to_landmarks
    ...
    for gi, item in enumerate(target.gloss):
        if item.fingerspelled:
            ...
            for li, ch in enumerate(letters):
                if li:
                    t += FS_GLIDE                # short glide between letters
...
def frames_json(target: SignTarget, fps: int = FPS) -> dict:
    ...
    for t in np.arange(0, total + 1, 1000 / fps):
        ...
        frames.append({
            ...
            "Rq": _pose_params(r), "Lq": _pose_params(l),      # for a 3D avatar (IK driven)
            "gi": gi, "ch": letter,
